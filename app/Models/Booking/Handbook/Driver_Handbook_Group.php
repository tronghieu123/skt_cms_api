...
namespace App\Models\Booking\Handbook;

use MongoDB\Laravel\Eloquent\Model;

class Driver_Handbook_Group extends Model
{
    public $timestamps = false;

    protected $connection = 'sky_booking';

    protected $table = 'driver_handbook_group';
...
//    }
    ...
    public function sub()
    {
        return $this->hasMany(Driver_Handbook_Group::class, 'parent_id', '_id')
            ->where('is_show', 1)
    ...
    function manager($paginate = 0, $filter = []) {
        ...
        if($paginate == 1){
            $data = Driver_Handbook_Group::when(request('sub') == 'manage' ?? null, function ($query){
